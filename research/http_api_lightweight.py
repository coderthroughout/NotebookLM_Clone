"""
Lightweight HTTP API for Phase 2 Context Search.
Uses OpenAI embeddings to avoid dependency conflicts.
"""
...
import uvicorn
from .lightweight_vector_index import lightweight_vector_index
...
app = FastAPI(
    title="Lightweight Research Context Search API",
    description="HTTP API for RAG retrieval using OpenAI embeddings",
    version="1.0.0"
)
...
async def root():
    ...
    return {
        "message": "Lightweight Research Context Search API",
        "version": "1.0.0",
...
async def context_search(
    ...
    """
    Search for relevant context using hybrid ranking with OpenAI embeddings.
    ...
    try:
        ...
        try:
            ...
            search_results = await lightweight_vector_index.search_with_context(q, k, db_conn)
            ...
            index_stats = lightweight_vector_index.get_index_stats()
...
async def get_index_stats():
    ...
    try:
        return lightweight_vector_index.get_index_stats()
    except Exception as e:
...
async def rebuild_index():
    ...
    try:
        db_conn = init_db()
        try:
            sections_added = await lightweight_vector_index.rebuild_index(db_conn)
            return {
...
async def hybrid_search(
    ...
    """
    Advanced hybrid search with custom weights using OpenAI embeddings.
    ...
    try:
        ...
        try:
            ...
            search_results = await lightweight_vector_index.search_hybrid(q, k, db_conn, weights)
...
async def test_embeddings():
    ...
    try:
        ...
        embeddings = await lightweight_vector_index.embed_texts(test_texts)
        ...
        return {
            ...
            "embedding_dimension": lightweight_vector_index.dimension,
...
def run_api_server(host: str = "0.0.0.0", port: int = 8000, reload: bool = False):
    """Run the lightweight FastAPI server."""
    uvicorn.run(
        "research.http_api_lightweight:app",
        host=host,
        port=port,
        reload=reload,
        log_level="info"
