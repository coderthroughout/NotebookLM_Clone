"""
Lightweight Vector Indexing Module for Phase 2.
Uses OpenAI embeddings to avoid dependency conflicts while maintaining full functionality.
"""
...
class LightweightVectorIndex:
    """Lightweight FAISS-based vector index using OpenAI embeddings."""
    
    def __init__(self, dimension: int = None, index_path: str = None):
        self.dimension = dimension or 1536  # OpenAI text-embedding-3-small dimension
        self.index_path = index_path or "lightweight_vector_index.faiss"
    ...
    def _load_index(self):
        ...
        if os.path.exists(self.index_path):
            try:
                print(f"Loading existing lightweight vector index from {self.index_path}")
    ...
    def _save_index(self):
        ...
        try:
            ...
            print(f"Saved lightweight vector index to {self.index_path}")
        except Exception as e:
    ...
    async def add_sections(self, doc_id: int, sections: List[Dict[str, Any]]) -> int:
        """
        ...
        if self.bm25_index is None:
            try:
                ...
                print("BM25 not available, skipping text-based ranking")
                self.bm25_index = None
                self.bm25_texts = []
        else:
        ...
        print(f"Added {len(texts)} sections from document {doc_id} to lightweight vector index")
        return len(texts)
    
    async def search_hybrid(self, query: str, k: int = 12, db_conn=None, 
    ...
    def _apply_diversity_controls(self, results: List[Dict], k: int) -> List[Dict]:
        ...
        # Apply SimHash deduplication if available
        try:
            from simhash import Simhash
            deduped_results = self._apply_simhash_dedup(results)
        except ImportError:
            print("SimHash not available, skipping deduplication")
    ...
    def _apply_simhash_dedup(self, results: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print("SimHash not available, returning original results")
            return results
    ...
    async def search_with_context(self, query: str, k: int = 12, db_conn=None) -> List[Dict[str, Any]]:
        """
        Search and return full context for each result.
        Uses hybrid ranking for better results.
        """
        # Use hybrid search
        search_results = await self.search_hybrid(query, k, db_conn)
    ...
    def get_index_stats(self) -> Dict[str, Any]:
        """Get statistics about the lightweight vector index."""
        return {
            ...
            'type': 'lightweight_openai'
        }
    
    async def rebuild_index(self, db_conn) -> int:
        """
        Rebuild the entire lightweight vector index from database content.
        """
        print("Rebuilding lightweight vector index from database...")
        ...
        for doc_id in doc_ids:
            sections = get_sections_by_doc_id(db_conn, doc_id)
            sections_added = await self.add_sections(doc_id, sections)
            total_sections += sections_added
        
        # Save the rebuilt index
        self._save_index()
        
        print(f"Rebuilt lightweight index with {total_sections} sections from {len(doc_ids)} documents")
        return total_sections
    
    def close(self):
        """Save and close the lightweight vector index."""
        self._save_index()
        print("Lightweight vector index saved and closed")


# Global lightweight vector index instance
lightweight_vector_index = LightweightVectorIndex()
